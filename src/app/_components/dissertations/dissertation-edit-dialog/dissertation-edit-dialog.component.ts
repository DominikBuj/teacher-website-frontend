...
export class DissertationEditDialogComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.dissertationForm = this.formBuilder.group({
      topic: [this.dissertation?.topic, [Validators.required, Validators.maxLength(200)]],
      status: [{value: this.dissertation?.status || DissertationStatus.Proposed, disabled: this.operation === Operation.Add}],
      date: [{value: this.dissertation?.date, disabled: this.dissertation?.status !== DissertationStatus[DissertationStatus.Completed]}],
      description: [this.dissertation?.description, [Validators.maxLength(2000)]]
    });
  }
